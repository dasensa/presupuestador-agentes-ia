...
export default async function handler(req, res) {
  ...
  try {
    const userMail = await resend.emails.send({
      ...
      html: `<div style="font-family:Arial,sans-serif;max-width:680px;margin:auto;color:#0f172a"><h1>Tu simulacion AgentIA</h1><p>Hola ${escapeHtml(nombre)},</p><p>Este es el resumen orientativo para ${escapeHtml(empresa)}, sector ${escapeHtml(sector)}. Un consultor puede validar el alcance antes de tomar una decision.</p><div style="background:#eff6ff;padding:18px;border-radius:14px;margin:24px 0"><small>INVERSION ESTIMADA ANO 1</small><br><strong>${currency(adjusted.inversion)}</strong></div><table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:10px">Agente</th><th style="text-align:left;padding:10px">Tipo</th><th style="text-align:right;padding:10px">Coste base ano 1</th></tr></thead><tbody>${rows}</tbody></table><p style="margin-top:24px;color:#64748b;font-size:12px">El ROI no se calcula a partir del precio. Requiere volumen operativo, valor por caso, alcance, efectividad, adopcion y consumos variables. Esta estimacion no constituye una oferta contractual ni una garantia de resultados.</p></div>`,
    });
