...
import { calculateMonthlyUsageCost, getAgentPricing } from './agent-pricing.js';
...
export const calcROIModel = ({ category, vars, initialCost, monthlyCost, pricing, usageVolume = 0, scenario, factors }) => {
  const beneficioMensual = calcBeneficioMensual(category, vars, scenario, factors);
  const monthlyOperationCost = pricing
    ? calculateMonthlyUsageCost(pricing, usageVolume).monthlyTotal
    : monthlyCost;
  const costePrimerAno = initialCost + (monthlyOperationCost * 12);
  ...
  const margenMensual = beneficioMensual - monthlyOperationCost;
  const paybackMonths = margenMensual > 0 ? initialCost / margenMensual : null;
  return { beneficioMensual, beneficioAnual, monthlyOperationCost, costePrimerAno, beneficioNeto, roi, paybackMonths };
};
...
export const calcResumenPersonalizado = (selectedCasos, variablesPorCategoria, factors = {}) => {
  ...
  selectedCasos.forEach((caso) => {
    ...
    categorias[cat].inversionAnual += pricing.initial + (pricing.baseMonthly * 12);
  });
